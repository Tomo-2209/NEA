...
The program's object-oriented tile hierarchy and MVC design pattern provide a solid foundation for future extension.

**Additional tile types.** A new tile type — for example a "freeze tile" — could be added by creating a class that extends the abstract `Tile` class and overrides its `draw()` and `onClick()` methods. A correct answer on a freeze tile would set a flag in `GameController` that skips the opponent's next turn, requiring only a small addition to the existing turn-management logic. The number of freeze tiles per grid size could follow the same lookup-table pattern already used by `BombManager`.

**Online multiplayer.** The game currently requires two players to share one machine. Because `GameController` separates all game logic from the view, a `RemotePlayer` class could be introduced alongside the existing `Player` class to relay moves from another machine, while the controller continues to call the same methods regardless of whether a player is local or remote. This would allow competitive play over a network with minimal restructuring of the existing code.

**Persistent leaderboard.** Player statistics are held in memory and lost when the application closes. Since `Player` is already a self-contained data class, its values could be written to a file at the end of each session and reloaded on launch. A leaderboard panel added to the start-up sequence would then let players compare their best-ever streaks across multiple sessions without altering any existing game logic.
